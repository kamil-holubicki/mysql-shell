...
//@<OUT> util dumpBinlogs help
NAME
      dumpBinlogs - Dumps binary logs generated since a specific point in time
...
DESCRIPTION
      ...
      - since: string (default: not set) - URL to a local or remote directory
        containing dump created either by util.dumpInstance() or
        util.dumpBinlogs(). Binary logs since the specified dump are going to
        be dumped.
      ...
      - ignoreDdlChanges: bool (default: false) - Proceeds with the dump even
        if the since option points to a dump created by util.dumpInstance()
      ...
      util.dumpBinlogs(), or by setting the since option to a directory
      containing dump created either by util.dumpInstance() or
      util.dumpBinlogs()) or as a explicitly specified binlog file and position
      in the startFrom option.

      Use util.loadBinlogs() to apply binary log dumps created with this
      command.
...
DESCRIPTION
      ...
      lock_invalid_accounts - Locks accounts which do not have a password or
      ...
      authentication plugin, are assigned an invalid password, and are
      initially locked.
...
//@<OUT> util loadBinlogs help
NAME
      loadBinlogs - Loads binary log dumps created by MySQL Shell from a local
                    or remote directory.
